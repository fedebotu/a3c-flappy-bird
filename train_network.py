...
from plots import plot_reward, plot_loss
...
from keras.optimizers import Adam #changed from RMSProp
...
newpath = "saved_models/" + timestr # makes folder for this training session
...
def buildmodel():
	...
	adam = Adam()
	model.compile(loss = {'o_P': logloss, 'o_V': sumofsquares}, loss_weights = {'o_P': 1., 'o_V' : 0.5}, optimizer = adam)
	return model
...
model = buildmodel()
# model = load_model("saved_models/20191207-2045/model_updates27200", custom_objects={'logloss': logloss, 'sumofsquares': sumofsquares})
...
for i in range(0,THREADS):
	game_state.append(game.GameState(30000, False))
...
def runprocess(thread_id, s_t):
	...
	while t-t_start < t_max and terminal == False:
		...
		x_t = preprocess(x_t) #x_t is the state

		with graph.as_default():
			critic_reward = model.predict(s_t)[1]

		y = 0 if a_t[0] == 1 else 1
		# print("Reward from function: ", r_t)
...
def step_decay(epoch):
	decay = 1.6e-8 #from 3.2e-8
	lrate = LEARNING_RATE - epoch*decay
	lrate = max(lrate, 6.4e-8) #from 3.2e-8
	return lrate
